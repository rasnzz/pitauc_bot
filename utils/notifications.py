...
async def send_outbid_notification(bot, user: User, auction: Auction, new_bid: float):
    ...
    try:
        message = (
            ...
            f"🔗 Ссылка: https://t.me/c/{str(Config.CHANNEL_ID).replace('@', '')}/{auction.channel_message_id}"
        )
        ...
        print(f"Ошибка при отправке уведомления о перебитии: {e}")
    ...
    async with get_db() as session:
        ...
        for subscription in subscriptions:
            try:
                ...
                print(f"Ошибка при уведомлении подписчика {subscription.user_id}: {e}")
...
async def send_winner_notification(bot, auction: Auction, winner: User):
    ...
    try:
        ...
        print(f"Ошибка при уведомлении победителя: {e}")
